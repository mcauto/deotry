...
func init() {
	...
	stdFileName := fmt.Sprintf("/var/log/%s/%s.log", config.Conf.Name, config.Conf.Name)
	// errFileName error file log path
	errFileName := fmt.Sprintf("/var/log/%s/%s_error.log", config.Conf.Name, config.Conf.Name)
